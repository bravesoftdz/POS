...
uses
  ...
  ShellAPI, StdCtrls, dxSkinsdxStatusBarPainter, dxStatusBar;
...
type
  ...
  TMainForm = class(TForm)
    ...
    taLokasiRak: TAction;
    taJnsHarga: TAction;
    N633JenisHarga1: TMenuItem;
    N634HargaJual1: TMenuItem;
    ...
    taNOUrut: TAction;
    taNOUrutSales: TAction;
    N637NoUrut1: TMenuItem;
    N624NoUrut1: TMenuItem;
    taNOUrutInv: TAction;
    N616NoUrut1: TMenuItem;
    N11: TMenuItem;
    N625HistoriHutang1: TMenuItem;
    taMutasiHutang: TAction;
    ...
    procedure taJnsHargaExecute(Sender: TObject);
    procedure FormShow(Sender: TObject);
    procedure taLokasiRakExecute(Sender: TObject);
    procedure taNOUrutInvExecute(Sender: TObject);
    procedure taNOUrutExecute(Sender: TObject);
    procedure taNOUrutSalesExecute(Sender: TObject);
    procedure taMutasiHutangExecute(Sender: TObject);
  private
...
uses Merk, Kategori, Rak, Barang, Golongan, Jenis, Cabang, Gudang,
  ...
  KeteranganGambar, MinMax, HBeli, JHarga, HJual, Login, lokasiRak, Supplier,
  INNoUrut, APNoUrut, ARNoUrut, historiStock, CreateMenu;
...
begin
  ...
  try
    ...
    adQueryVersion.Connection := connTrading;
...
begin
  ...
  if FileExists('UserLog.sys') then DeleteFile('UserLog.sys');
end;
...
var s, i : integer;
    ...
    constr:String;
begin
  if not IsMDIChildOpen(MainForm, 'LoginForm') then
    f := TLoginForm.Create(Self);
   If FileExists(sSys) then
   Begin
     mmo1.Lines.LoadFromFile('UserLog.Sys');
     cUser:=Trim(mmo1.Lines.Strings[0]);
     cPass:=Trim(mmo1.Lines.Strings[1]);
     with spUser do
     begin
          close;
     ...
     begin
       if spUserGroupUser.Value = 'ADMINISTRA' then
       begin
//         MenuCreate.ShowModal;
         if not IsMDIChildOpen(MainForm, 'CreateMenuForm') then
            CreateMenuForm := TCreateMenuForm.Create(Self);

            CreateMenuForm.Show;
            CreateMenuForm.BringToFront;//
       end else
       begin
//          BuildMainMenu;
         dxStatusBar1.Panels.Items[3].Text := spUserNm_user.Value;
         dxStatusBar1.Panels.Items[2].Text := FormatDateTime('YYYY-MM-DD HH:MM:SS', now);
       end;
     End Else
     begin
       ShowMessage('User Id / Password tidak terdaftar !');
     end;
   end Else
   if f.ShowModal = mrOk then
   begin
     Cuser :=  f.edtUserName.Text;
     Cpass :=  f.edtPassword.Text;
     with spUser do
     begin
        close;
        Parameters.ParamByName('userIdIn').Value :=  cUser;
     ...
     begin
       ...
       MyList.Add(Cuser +#13#10+cpass);
       constr := MyList.Text;
       MyList.Free;
       AssignFile(Ft,ExtractFilePath(ParamStr(0))+sSYS);
       Rewrite(Ft);
       Writeln(Ft,constr);
       CloseFile(Ft);
       if spUserGroupUser.Value = 'ADMINISTRA' then
...
//         MenuCreate.ShowModal;
       ...
       begin
//          BuildMainMenu;
         dxStatusBar1.Panels.Items[2].Text := spUserNm_user.Value;
//         dxStatusBar1.Panels.Items[2].Text := FormatDateTime('YYYY-MM-DD HH:MM:SS', now);
       end;
     end else
     begin
...
//              BuildMainMenu;
             dxStatusBar1.Panels.Items[2].Text := spUserNm_user.Value;
//             dxStatusBar1.Panels.Items[3].Text := FormatDateTime('YYYY-MM-DD HH:MM:SS', now);
           end;
         End Else
         begin
           ShowMessage('User Id / Password tidak terdaftar !');
...
begin
  if not IsMDIChildOpen(MainForm, 'CabangForm') then
    CabangForm := TCabangForm.Create(Self);

    CabangForm.Show;
    CabangForm.BringToFront;//
end;
...
begin
  if not IsMDIChildOpen(MainForm, 'GolonganForm') then
    GolonganForm := TGolonganForm.Create(self);

    GolonganForm.show;
    GolonganForm.BringToFront;//
end;

procedure TMainForm.taGudangExecute(Sender: TObject);
begin
  if not IsMDIChildOpen(MainForm, 'GudangForm') then
    GudangForm := TGudangForm.Create(self);

    GudangForm.show;
    GudangForm.BringToFront;//
end;

procedure TMainForm.taHargaBeliExecute(Sender: TObject);
begin
  if not IsMDIChildOpen(MainForm, 'HBeliForm') then
    HBeliForm := THBeliForm.Create(self);

    HbeliForm.show;
    HBeliForm.BringToFront;//
end;

procedure TMainForm.taHargaJualExecute(Sender: TObject);
begin
  if not IsMDIChildOpen(MainForm, 'HJualForm') then
    HJualForm := THJualForm.Create(self);

    HJualForm.show;
    HJualForm.BringToFront;
end;

procedure TMainForm.taMutasiHutangExecute(Sender: TObject);
begin
  EksekusiMenu(Sender);
end;
...
procedure TMainForm.taJnsHargaExecute(Sender: TObject);
begin
  if not IsMDIChildOpen(MainForm, 'JHargaForm') then
    JHargaForm := TJHargaForm.Create(self);

    JHargaForm.show;
    JHargaForm.BringToFront;//
end;

procedure TMainForm.taKategoriExecute(Sender: TObject);
begin
  if not IsMDIChildOpen(MainForm, 'KategoriForm') then
    KategoriForm := TKategoriForm.Create(self);

    KategoriForm.show;
    KategoriForm.BringToFront;//
end;

procedure TMainForm.taKeteranganExecute(Sender: TObject);
begin
  if not IsMDIChildOpen(MainForm, 'KeteranganGambarForm') then
    KeteranganGambarForm := TKeteranganGambarForm.Create(self);

    KeteranganGambarForm.show;
    KeteranganGambarForm.BringToFront;//
end;

procedure TMainForm.taLokasiRakExecute(Sender: TObject);
begin
  if not IsMDIChildOpen(MainForm, 'LokasiRakForm') then
    LokasiRakForm := TLokasiRakForm.Create(self);

    LokasiRakForm.show;
    LokasiRakForm.BringToFront;//
end;

procedure TMainForm.taMerkExecute(Sender: TObject);
begin
  if not IsMDIChildOpen(MainForm, 'MerkForm') then
    MerkForm := TMerkForm.Create(self);

    MerkForm.show;
    MerkForm.BringToFront;//
end;

procedure TMainForm.taMinMaxExecute(Sender: TObject);
begin
  if not IsMDIChildOpen(MainForm, 'MinMaxForm') then
    MinMaxForm := TMinMaxForm.Create(self);

    MinMaxForm.show;
    MinMaxForm.BringToFront;//
end;
...
begin
  if not IsMDIChildOpen(MainForm, 'historiStockForm') then
    historiStockForm := ThistoriStockForm.Create(self);

    historiStockForm.show;
    historiStockForm.BringToFront;//
end;

procedure TMainForm.taNOUrutExecute(Sender: TObject);
begin
   if not IsMDIChildOpen(MainForm, 'APNOUrutForm') then
    APNOUrutForm := TAPNOUrutForm.Create(self);

    APNOUrutForm.show;
    APNOUrutForm.BringToFront;//
end;

procedure TMainForm.taNOUrutInvExecute(Sender: TObject);
begin
   if not IsMDIChildOpen(MainForm, 'INNOUrutForm') then
    INNOUrutForm := TINNOUrutForm.Create(self);

    INNOUrutForm.show;
    INNOUrutForm.BringToFront;//
end;

procedure TMainForm.taNOUrutSalesExecute(Sender: TObject);
begin
  if not IsMDIChildOpen(MainForm, 'ARNOUrutForm') then
    ARNOUrutForm := TARNOUrutForm.Create(self);

    ARNOUrutForm.show;
    ARNOUrutForm.BringToFront;//
end;
...
begin
  if not IsMDIChildOpen(MainForm, 'RakForm') then
    RakForm := TRakForm.Create(self);

    RakForm.show;
    RakForm.BringToFront;//
end;
...
begin
  if not IsMDIChildOpen(MainForm, 'SaldoAwalBarangForm') then
    SaldoAwalBarangForm := TSaldoAwalBarangForm.Create(self);

    SaldoAwalBarangForm.show;
    SaldoAwalBarangForm.BringToFront;//
end;
...
begin
  if not IsMDIChildOpen(MainForm, 'ValutaPurchaseForm') then
    ValutaPurchaseForm := TValutaPurchaseForm.Create(self);

    ValutaPurchaseForm.show;
    ValutaPurchaseForm.BringToFront;//
end;

procedure TMainForm.taValutaSalesExecute(Sender: TObject);
begin
  if not IsMDIChildOpen(MainForm, 'ValutaSalesForm') then
    ValutaSalesForm := TValutasalesForm.Create(self);

    ValutaSalesForm.show;
    ValutaSalesForm.BringToFront;//
end;

procedure TMainForm.taWilayahPurchaseExecute(Sender: TObject);
begin
  if not IsMDIChildOpen(MainForm, 'WilayahPurchaseForm') then
    WilayahPurchaseForm := TWilayahPurchaseForm.Create(self);

    WilayahPurchaseForm.show;
    WilayahPurchaseForm.BringToFront;//
end;

procedure TMainForm.taWilayahSalesExecute(Sender: TObject);
begin
  if not IsMDIChildOpen(MainForm, 'WilayahSalesForm') then
    WilayahSalesForm := TwilayahSalesForm.Create(self);

    WilayahSalesForm.show;
    WilayahSalesForm.BringToFront;//
end;
