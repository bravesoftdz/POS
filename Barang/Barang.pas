...
type
  TBarangForm = class(TtemplateForm)
    ...
    procedure Baranghjual3Validate(Sender: TField);
    procedure Baranghjual2Validate(Sender: TField);
    procedure BarangNewRecord(DataSet: TDataSet);
    procedure BarangBRGIDValidate(Sender: TField);
    procedure BarangBeforePost(DataSet: TDataSet);
  private
...
procedure TBarangForm.BarangBeforePost(DataSet: TDataSet);
begin
  inherited;
  Barangcab.Value := system_CabID.Value;
end;

procedure TBarangForm.BarangBRGIDValidate(Sender: TField);
begin
  inherited;
  Barangbarcode.Value := BarangBRGID.Value;
end;
...
procedure TBarangForm.Baranghjual2Validate(Sender: TField);
begin
  inherited;
  if Baranghjual2.AsCurrency > 0  then
  begin
    Baranghjual1.AsCurrency := Baranghjual2.AsCurrency * BarangIsi1.AsCurrency;
  end;
end;

procedure TBarangForm.Baranghjual3Validate(Sender: TField);
begin
  inherited;
  if Baranghjual3.AsCurrency > 0  then
  begin
    Baranghjual2.AsCurrency := Baranghjual3.AsCurrency * BarangIsi2.AsCurrency;
    Baranghjual1.AsCurrency := Baranghjual2.AsCurrency * BarangIsi1.AsCurrency;
  end;
...
procedure TBarangForm.BarangNewRecord(DataSet: TDataSet);
begin
  inherited;
  BarangIsi1.AsCurrency := 1;
  BarangIsi2.AsCurrency := 1;
  Baranghbeli.AsCurrency := 0;
  Baranghjual1.AsCurrency := 0;
  Baranghjual2.AsCurrency := 0;
  Baranghjual3.AsCurrency := 0;
end;
